...
async def exam_tag_question_handler(callback: types.CallbackQuery, callback_data: QuestionCallbackFactory, state: FSMContext):
    ...
    q_text = question['question_text_new']
...
async def answer_handler(message: Message, state: FSMContext):
    ...
    try:
        verdict = post_question_answer(
            token=token, answer=answer, question_id=chosen_question_id)
        score = verdict['score']
...
async def explanation_handler(callback: types.CallbackQuery, callback_data: QuestionCallbackFactory, state: FSMContext):
    ...
    question = get_question(question_id=chosen_question_id,
                            token=token, need_answer=True)
    ...
    true_options = list(
        filter(lambda option: option["is_true"], question['all_options']))
